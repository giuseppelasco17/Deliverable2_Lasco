...
public class Metrics {
	...
	public static TreeMap<Integer,List<String>> listAllCommitsPerVersion(String projName) {
		/*
		 * it retrieves all the commits of the project and collect them for version in a map
		 * */
		...
		//TODO vedi se ha senso inserire l'ultimo commit della versione i anche nella lista della i-1
	...
	public static List<CSVEntry> computeMetrics(TreeMap<Integer,List<String>> versionCommitsMap){
		/**
		 *it analyzes the differences between the i and i-1 commit with the purpose of compute all the metrics the project needs 
		 */
		...
		for(Entry<Integer,List<String>> entryMap : versionCommitsMap.entrySet()) {//for version
			...
			for(int i = 1; i < commitList.size(); i++) {//for commits in version k
				...
				for(String filePlusAttr : filesPlusAttrList) {//for file in difference between commit i and i-1
					...
					entry.setAvgLocAdded(actualAvgLocAdded * ((actualNR)/(actualNR + Double.valueOf(1))) + addedLines *
							(Double.valueOf(1)/(actualNR + Double.valueOf(1))));
					...
					entry.setAvgChurn(actualAvgChurn * ((actualNR)/(actualNR + Double.valueOf(1))) + (addedLines - removedLines) *
							(Double.valueOf(1)/(actualNR + Double.valueOf(1))));
					...
					entry.setAvgChgSet(actualAvgChgSet * ((actualNR)/(actualNR + Double.valueOf(1))) + noOfFiles *
							(Double.valueOf(1)/(actualNR + Double.valueOf(1))));
